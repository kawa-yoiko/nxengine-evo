...
void HandlePlayer(void)
{
  ...
  if (player->riding && !player->lastriding && (player->riding->nxflags & NXFLAG_THUD_ON_RIDING))
  {
    rumble(0.3, 100);
...
void HandlePlayer_am(void)
{
  ...
  else if (player->blocku && player->yinertia < 0)
  {
    ...
    if (player->yinertia < -0x200 && !player->hide && player->blocku == BLOCKED_MAP)
    {
...
void PHandleAttributes(void)
{
  ...
  if (attr & TA_WATER)
  {
    ...
    if (!(player->touchattr & TA_WATER))
    {
      ...
      if ((player->yinertia > 0x200 && !player->blockd) || (player->xinertia < -0x200 || player->xinertia > 0x200))
      {
        int x          = player->CenterX();
        int y          = player->CenterY();
        int splashtype = !(player->touchattr & TA_HURTS_PLAYER) ? OBJ_WATER_DROPLET : OBJ_LAVA_DROPLET;
...
{
  static Point currentpoints[] = {{7, 8}, {1, 2}, {1, 8}, {1, 14}, {7, 2}, {7, 14}, {15, 2}, {15, 8}, {15, 14}};
  int i;
  ...
  if (currentmask & LEFTMASK)
    player->xinertia -= 0x88;
  if (currentmask & RIGHTMASK)
    player->xinertia += 0x88;
...
void PDoFalling(void)
{
  ...
  if (game.curmap == STAGE_KINGS_TABLE && fade.getstate() == FS_FADING)
    return;
...
void PDoLooking(void)
{
  ...
  if (player->lookaway)
  {
    ...
    for (i = 0;; i++)
    {
      ...
      if (key == INPUT_COUNT)
        break;
      if (!settings->strafing && key == STRAFEKEY)
        break;
...
void PDoBooster(void)
{
  ...
  switch (player->booststate)
  {
    case BOOST_HOZ:
    {
      if ((player->dir == LEFT && player->blockl) || (player->dir == RIGHT && player->blockr))
      {
...
{
  if (RunScriptAtLocation(x, player->y + (8 * CSFI)) || RunScriptAtLocation(x, player->y + (14 * CSFI))
